...
// Pure draw function — no React state, no closures
function drawChart(canvas: HTMLCanvasElement, prices: number[], cursor: number, HISTORY: number) {
...
export default function TradeGame() {
    ...
    const cursorRef = useRef(HISTORY); // source of truth for drawing

    const [phase, setPhase] = useState<Phase>("intro");
    const [round, setRound] = useState(0);
    const [displayCursor, setDisplayCursor] = useState(HISTORY);
    const [portfolio, setPortfolio] = useState(10000);
    const [aiPortfolio, setAiPortfolio] = useState(10000);
    const [position, setPosition] = useState<"none" | "long">("none");
    const [feedback, setFeedback] = useState<string | null>(null);

    const repaint = (cursor: number) => {
        // rAF garantit que le canvas est peint après que React a mis à jour display:block
        requestAnimationFrame(() => {
            const canvas = canvasRef.current;
            if (canvas && pricesRef.current.length > 1) {
                drawChart(canvas, pricesRef.current, cursor, HISTORY);
            }
        });
    };

    const startGame = () => {
        const prices = generatePrices(HISTORY + ROUNDS + 5);
        pricesRef.current = prices;
        cursorRef.current = HISTORY;
        setRound(0); setPortfolio(10000); setAiPortfolio(10000);
        setPosition("none"); setFeedback(null);
        setDisplayCursor(HISTORY);
        setPhase("playing");
        // Dessiner directement après que React a commité le DOM
        repaint(HISTORY);
    };
    ...
    const decide = (decision: Decision) => {
        ...
        const cur = prices[c], nxt = prices[c + 1] ?? cur;
        ...
        const aiDec = aiDecision(prices, c);
    ...
    return (
        <section id="trade" className="relative py-32 px-6 overflow-hidden">
            ...
            <div className="max-w-4xl mx-auto">
                ...
                {phase === "intro" && (
                    <div className="glass rounded-2xl p-8 border border-yellow-400/15 text-center">
                        ...
                        <div className="space-y-2 text-sm text-white/50 mb-8 max-w-sm mx-auto">
                            ...
                            <p>🏆 Achievement si vous gagnez</p>
                        </div>
                        <button
                            onClick={startGame}
                ...
                {/* PLAYING — canvas TOUJOURS dans le DOM, visibility via CSS */}
                ...
                {phase === "result" && (
                    <div className="glass rounded-2xl p-8 border border-white/10 text-center">
                        ...
                        <p className="text-white/40 text-sm mb-6 max-w-md mx-auto">Stratégie momentum : achat sur hausse, vente sur baisse.</p>
