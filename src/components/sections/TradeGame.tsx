...
export default function TradeGame() {
    ...
    const drawChart = useCallback(() => {
        ...
        // Fill
        const lastX = ((slice.length - 1) / (slice.length - 1)) * W;
        const lastY = H - ((slice[slice.length - 1] - min) / (max - min)) * H;
    ...
    // Draw whenever cursor or phase changes
    useEffect(() => {
    ...
    const startGame = () => {
        const data = generatePriceData(HISTORY + TOTAL_ROUNDS + 5);
    ...
    const makeDecision = (decision: Decision) => {
        const prices = pricesRef.current;
        const curPrice = prices[cursor];
        const nextPrice = prices[cursor + 1] ?? curPrice;
        const pct = (nextPrice - curPrice) / curPrice;
        ...
        const fb: Record<Decision, string> = {
        ...
        setFeedback(fb[decision]);
        setTimeout(() => setFeedback(null), 1400);

        setPortfolio(newP);
        setAiPortfolio(newAi);
        setPosition(newPos);
        setCursor((c) => c + 1);
            ...
            <div className="max-w-4xl mx-auto">
                ...
                >
                    ...
                    <p className="text-white/40 max-w-md mx-auto text-sm">
                        Pouvez-vous battre mon algorithme ? 8 décisions. 1 seul gagnant.
                    </p>
                </motion.div>
                ...
                {phase === "intro" && (
                    <div className="glass rounded-2xl p-8 border border-yellow-400/15 text-center">
                        ...
                        <button
                            onClick={startGame}
                            className="px-8 py-3 rounded-xl font-bold text-black bg-gradient-to-r from-yellow-400 to-orange-400 hover:opacity-90 active:scale-95 transition-all cursor-pointer"
                        >
                            Commencer la partie →
                        </button>
                    </div>
                ...
                {phase === "playing" && (
                    <div className="space-y-4">
                        ...
                        <div className="text-center text-xs font-mono text-white/30">
                            Position : <span className={position === "long" ? "text-emerald-400" : "text-white/40"}>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            {(["BUY", "SELL", "HOLD"] as Decision[]).map((dec) => (
                                <button
                                    ...
                                    className={`py-4 rounded-xl font-bold text-sm border transition-all cursor-pointer active:scale-95 ${
                                        dec === "BUY"
                ...
                {phase === "result" && (
                    <div className="glass rounded-2xl p-8 border border-white/10 text-center">
                        ...
                        <p className="text-white/40 text-sm mb-6 max-w-md mx-auto">
                            Mon algorithme utilise une stratégie momentum : achat quand le prix monte, vente quand il baisse.
                        </p>
                        <div className="flex items-center justify-center gap-4">
                            <button
                                onClick={startGame}
                                className="px-6 py-3 rounded-xl border border-white/15 text-white/60 hover:text-white hover:border-white/30 text-sm font-medium transition-all cursor-pointer active:scale-95"
                            >
                                Rejouer
                            </button>
                            <button
                                onClick={() => document.querySelector("#projects")?.scrollIntoView({ behavior: "smooth" })}
                                className="px-6 py-3 rounded-xl font-bold text-sm bg-gradient-to-r from-cyan-400 to-violet-500 text-black hover:opacity-90 transition-all cursor-pointer active:scale-95"
                            >
                                Voir mes projets →
                            </button>
                        </div>
